...
import { FloatLabel, JsonPreviewer } from "freewind-ts-utils";
import type {
...
const LabeledField: FC<{
  ...
  children: ReactNode;
}> = ({ name, label, className, rules, children }) => {
  return (
...
function toSnapshotPreviewNodes(
...
function buildSnapshotTree(
  ...
  const byParent = nodes.reduce<Record<string, SnapshotResponse["nodes"]>>(
    (result, node) => {
  ...
  const toTreeNode = (
    node: NonNullable<SnapshotResponse["nodes"]>[number],
  ): SnapshotTreeNode => {
...
const SnapshotPreview: FC<{ snapshot: SnapshotResponse | null }> = ({
  snapshot,
}) => {
  ...
  const maxRight = Math.max(
    ...nodes.map((item) => item.bounds.left + item.bounds.width),
  );
  const maxBottom = Math.max(
    ...nodes.map((item) => item.bounds.top + item.bounds.height),
  );
  ...
  return (
    <div ref={shellRef} className="snapshot-preview-shell">
      <div className="snapshot-preview-bar">
        <Text type="secondary">
          {snapshot?.screen || snapshot?.summary?.screen || "Unknown"} ·{" "}
          {nodes.length} nodes
        </Text>
      </div>
      <div className="snapshot-preview-canvas" style={{ height: canvasHeight }}>
        {nodes.map((node) => {
          ...
          const fontSize = Math.max(
            7,
            Math.min(14, Math.floor(scaledHeight * 0.5)),
          );
          ...
          return (
            ...
            >
              {showLabel ? (
                <>
                  {!isContainer ? (
                    <div className="snapshot-preview-node-label">{label}</div>
                  ) : null}
                  {!isTextLike && !isContainer ? (
                    <div className="snapshot-preview-node-meta">
                      {typeLabel}
                    </div>
                  ) : null}
                </>
...
const SnapshotTreeView: FC<{ snapshot: SnapshotResponse | null }> = ({
  snapshot,
}) => {
...
const App: FC = () => {
  ...
  const [actionResult, setActionResult] = useState<
    ActionResponse | LogsClearResponse | null
  >(null);
  ...
  const actionDescriptors = (
    actions?.items || []
  ).flatMap<ActionDescriptorView>((item) => {
  ...
  const actionDescriptorByKey = actionDescriptors.reduce<
    Record<string, ActionDescriptorView>
  >((result, item) => {
    result[buildActionKey(item.targetId, item.name)] = item;
    return result;
  }, {});
  const actionsByTargetId = (actions?.items || []).reduce<
  ...
  const selectedManualActionDescriptor =
    manualActionTargetId && manualActionAction
      ? actionDescriptorByKey[
          buildActionKey(manualActionTargetId, manualActionAction)
        ]
      : undefined;
  const manualActionArgNames = (
    selectedManualActionDescriptor?.args || []
  ...
  function hydrateManualAction(targetId: string, action: string) {
    ...
    const argValues = descriptor.args.reduce<Record<string, string>>(
      (result, argName) => {
        if (argName === "text") {
          return result;
        }
    ...
    manualActionForm.setFieldsValue({
      targetId,
      action,
      text: descriptor.args.includes("text")
        ? (descriptor.example.text ?? undefined)
        : undefined,
  ...
  async function loadActions(values?: Record<string, unknown>) {
    ...
    const result = await requestJSON<ActionCatalogResponse>(
      `/action${buildQuery(formValues)}`,
    );
    setActions(result);
    return result;
  }
  ...
  async function loadLogs(values?: Record<string, unknown>) {
    ...
    const result = await requestJSON<LogsResponse>(
      `/logs${buildQuery(formValues)}`,
    );
    setLogs(result);
    return result;
  }
  ...
  async function loadSnapshot(values?: Record<string, unknown>) {
    const formValues = normalizeSnapshotQuery(
      values ?? snapshotForm.getFieldsValue(),
    );
    const result = await requestJSON<SnapshotResponse>(
      `/snapshot${buildQuery(formValues)}`,
    );
    setSnapshot(result);
    return result;
  }
    ...
    try {
      await Promise.all([
        ...
        fetchJSON<SnapshotResponse>(
          `/snapshot${buildQuery(normalizeSnapshotQuery(snapshotForm.getFieldsValue()))}`,
          { baseURL },
        ),
      ]).then(
        ([
  ...
  async function runManualAction() {
    try {
      const values =
        (await manualActionForm.validateFields()) as ManualActionFormValues;
      const args = Object.entries(values.argValues || {}).reduce<
        Record<string, string>
      >((result, [key, rawValue]) => {
  ...
  async function clearLogs() {
    try {
      const result = await requestJSON<LogsClearResponse>("/logs", {
        method: "DELETE",
      });
      setActionResult(result);
      message.success(result.message);
  ...
  useEffect(() => {
    ...
    const matched = actionQueryActionOptions.some(
      (item) => item.value === actionQueryAction,
    );
    if (!matched) {
  ...
  useEffect(() => {
    ...
    const matched = manualActionOptions.some(
      (item) => item.value === manualActionAction,
    );
    if (!matched) {
      ...
      <Header className="page-header">
        <Flex justify="space-between" align="center" gap={12} wrap>
          <Space direction="vertical" size={2}>
            <Title
              level={4}
              style={{ margin: 0, lineHeight: "32px", paddingTop: 6 }}
            >
              {consoleTitle}
            </Title>
            <Space size={8} wrap>
              <Text strong className="header-app-name">
                {headerAppName}
              </Text>
        ...
        <Space direction="vertical" size={8} style={{ display: "flex" }}>
          ...
          <Flex gap={8} wrap>
            ...
            <Card size="small" className="stat-card">
              <Statistic
                title="State Keys"
                value={help?.counts.stateKeyCount ?? 0}
              />
            </Card>
            <Card size="small" className="stat-card">
              <Statistic
                title="Snapshot Nodes"
                value={help?.counts.snapshotNodeCount ?? 0}
              />
            </Card>
          </Flex>
                        ...
                        <Flex vertical gap="small">
                          <Flex gap="small" wrap>
                            ...
                            <LabeledField
                              name="limit"
                              label="limit"
                              className="query-cell query-cell--number"
                            >
                              <InputNumber
                                size="small"
                          ...
                          <Flex gap="small" wrap>
                            ...
                            >
                              <Select
                                {...commonSelectProps}
                                allowClear
                                options={actionQueryActionOptions}
                              />
                            </LabeledField>
                            <LabeledField
                              name="screen"
                          ...
                          {selectedManualActionDescriptor ? (
                            <Space size={4} wrap>
                              ...
                              {selectedManualActionDescriptor.args.length ? (
                                selectedManualActionDescriptor.args.map(
                                  (argName) => (
                                    <Tag
                                      key={`manual-${argName}`}
                                      color={
                                        argName === "text" ? "gold" : "default"
                                      }
                                    >
                                      {argName}
                                    </Tag>
                          ...
                          <Flex gap="small" wrap>
                            ...
                            >
                              <Select
                                ...
                                onChange={(value) =>
                                  value && manualActionTargetId
                                    ? hydrateManualAction(
                                        manualActionTargetId,
                                        value,
                                      )
                                    : manualActionForm.setFieldValue(
                                        "action",
                                        value,
                                      )
                                }
                        ...
                        <Flex vertical gap="small">
                          <Flex gap="small" wrap>
                            <LabeledField
                              name="keys"
                              label="keys"
                              className="query-cell query-cell--wide"
                            >
                              <Select
                                {...commonSelectProps}
                                mode="multiple"
                          ...
                          <Flex gap="small" wrap>
                            ...
                            >
                              <Select
                                {...commonSelectProps}
                                allowClear
                                options={snapshotTargetIdOptions}
                              />
                            </LabeledField>
                            <LabeledField
                              name="scope"
